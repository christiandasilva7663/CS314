...
def assoc_list (l):
    ...
    return[(x,counts[x]) for x in counts]
...
def level_order(root: TreeNode):
    ...
    def level_order_helper(Newnode, level):
        if len(result) < level:
            result.append([])
        result[level-1].append(Newnode.val)

        if Newnode.left:
            level_order_helper(Newnode.left, level+1)
        if Newnode.right:
            level_order_helper(Newnode.right, level+1)

    level_order_helper(root,1)
    return result
...
def pathSum(root: TreeNode, targetSum: int) -> List[List[int]]:
    result = []
    def pathSum_helper(Newnode, currentSum, targetPath):
        currentSum += Newnode.val

        targetPath.append(Newnode.val)

        if not Newnode.left and not Newnode.right and currentSum == targetSum:
            result.append(targetPath[:])
        else:
            if Newnode.left:
                pathSum_helper(Newnode.left,currentSum,targetPath)

            if Newnode.right:
                pathSum_helper(Newnode.right, currentSum, targetPath)
